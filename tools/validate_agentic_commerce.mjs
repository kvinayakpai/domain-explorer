#!/usr/bin/env node
// Validate the agentic_commerce anchor YAML against the Zod schema, then
// run a wider sanity check on the registry as a whole. Prints a one-line
// PASS/FAIL token per check.
...
// Re-implement the relevant portions of the schema inline so we don't have
// to compile the TypeScript packages first.
...
const RelationshipKind = z.enum(["one_to_one", "one_to_many", "many_to_many"]);
const EntityRelationship = z.object({
  to: z.string().min(1),
  kind: RelationshipKind,
  via: z.string().optional(),
  from: z.string().optional(),  // tolerant
});
...
function fail(msg) {
  console.error("agentic_commerce: FAIL — " + msg);
  process.exit(1);
}

const repoRoot = resolve(process.argv[2] ?? ".");
const yamlPath = resolve(repoRoot, "data/taxonomy/agentic_commerce.yaml");
...
if (sd.id !== "agentic_commerce") fail(`id mismatch: ${sd.id}`);
if (sd.vertical !== "CrossCutting") fail(`vertical mismatch: ${sd.vertical}`);
...
console.log("agentic_commerce: OK");
