"""
Load Customer Loyalty & CDP parquet output into the populated DuckDB.
...
``synthetic-data/output/customer_loyalty_cdp/``.

Connects directly to the EXISTING ``domain-explorer.duckdb`` so other anchor
schemas stay intact. Only drops + recreates tables inside the
``customer_loyalty_cdp`` schema.

Run:
    python synthetic-data/customer_loyalty_cdp/load_to_duckdb.py
    python synthetic-data/customer_loyalty_cdp/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/customer_loyalty_cdp/generate.py first."
        )
