"""
Load Procurement & Spend Analytics parquet output into the populated DuckDB.

Mirrors ``synthetic-data/generate_all.py::load_into_duckdb`` —
``CREATE SCHEMA IF NOT EXISTS`` + ``CREATE TABLE ... AS SELECT * FROM
read_parquet(?)`` for every parquet under
``synthetic-data/output/procurement_spend_analytics/``.

Run:
    python synthetic-data/procurement_spend_analytics/generate.py --scale medium
    python synthetic-data/procurement_spend_analytics/load_to_duckdb.py

Honours the ``DOMAIN_EXPLORER_DUCKDB`` env var when ``--duckdb`` is omitted.
"""
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/procurement_spend_analytics/generate.py first."
        )
