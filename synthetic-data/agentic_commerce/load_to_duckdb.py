"""
Load Agentic Commerce parquet output into the populated DuckDB.
...
``synthetic-data/output/agentic_commerce/``.
...
``domain-explorer.duckdb`` so the other 17 anchor schemas stay intact. We only
drop + recreate tables inside the ``agentic_commerce`` schema.

Run:
    python synthetic-data/agentic_commerce/load_to_duckdb.py
    python synthetic-data/agentic_commerce/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/agentic_commerce/generate.py first."
        )
