"""
Load Omnichannel OMS parquet output into the populated DuckDB.
...
``synthetic-data/output/omnichannel_oms/``.

Connects directly to the EXISTING ``domain-explorer.duckdb`` so the other
anchor schemas stay intact. We only drop + recreate tables inside the
``omnichannel_oms`` schema.

Run:
    python synthetic-data/omnichannel_oms/load_to_duckdb.py
    python synthetic-data/omnichannel_oms/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/omnichannel_oms/generate.py first."
        )
