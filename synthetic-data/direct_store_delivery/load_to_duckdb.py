"""
Load Direct Store Delivery parquet output into the populated DuckDB.

Mirrors the trade_promotion_management / agentic_commerce loaders: connect to
the existing ``domain-explorer.duckdb`` (via ``--duckdb`` or
``DOMAIN_EXPLORER_DUCKDB``), ``CREATE SCHEMA IF NOT EXISTS
direct_store_delivery``, then drop + recreate every parquet under
``synthetic-data/output/direct_store_delivery/`` as a table.

Run:
    python synthetic-data/direct_store_delivery/load_to_duckdb.py
    python synthetic-data/direct_store_delivery/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
"""
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/direct_store_delivery/generate.py first."
        )
