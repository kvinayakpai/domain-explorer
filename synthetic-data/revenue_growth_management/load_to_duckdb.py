"""
Load Revenue Growth Management parquet output into the populated DuckDB.

Mirrors the pattern in ``synthetic-data/agentic_commerce/load_to_duckdb.py`` --
``CREATE SCHEMA IF NOT EXISTS`` + ``CREATE TABLE ... AS SELECT * FROM
read_parquet(?)`` for every parquet under
``synthetic-data/output/revenue_growth_management/``.

Run:
    python synthetic-data/revenue_growth_management/load_to_duckdb.py
    python synthetic-data/revenue_growth_management/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/revenue_growth_management/generate.py first."
        )
