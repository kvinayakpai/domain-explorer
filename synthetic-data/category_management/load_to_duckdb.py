"""
Load Category Management parquet output into the populated DuckDB.
...
``synthetic-data/output/category_management/``.
...
drop + recreate tables inside the ``category_management`` schema.

Run:
    python synthetic-data/category_management/load_to_duckdb.py
    python synthetic-data/category_management/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/category_management/generate.py first."
        )
