"""
Load Loss Prevention parquet output into the populated DuckDB.

Mirrors synthetic-data/agentic_commerce/load_to_duckdb.py:
  - CREATE SCHEMA IF NOT EXISTS "loss_prevention"
  - DROP / CREATE every table from output/loss_prevention/*.parquet
  - Honours DOMAIN_EXPLORER_DUCKDB env var (same as the dbt profile).

Run:
    python synthetic-data/loss_prevention/load_to_duckdb.py
    python synthetic-data/loss_prevention/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
"""
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/loss_prevention/generate.py first."
        )
