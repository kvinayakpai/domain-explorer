"""
Load Returns & Reverse Logistics parquet output into the populated DuckDB.
...
``synthetic-data/output/returns_reverse_logistics/``.

Connects directly to the EXISTING ``domain-explorer.duckdb`` so the other
anchor schemas stay intact. Only drops + recreates tables inside the
``returns_reverse_logistics`` schema.

Run:
    python synthetic-data/returns_reverse_logistics/load_to_duckdb.py
    python synthetic-data/returns_reverse_logistics/load_to_duckdb.py --duckdb /path/to/domain-explorer.duckdb
...
def load(duckdb_path: Path) -> int:
    if not OUT_DIR.exists():
        raise SystemExit(
            ...
            f"       Run synthetic-data/returns_reverse_logistics/generate.py first."
        )
